...
// Click-to-cycle order and per-state presentation.
const ORDER = ["unknown", "optimal", "great", "good", "bad", "bad+", "bad-"];
 ...
 */
export function FeedbackCycle({ value, onChange, disabled }) {
  const state = STATES[value] || STATES.unknown;

  const step = (dir) => {
    const i = ORDER.indexOf(value in STATES ? value : "unknown");
