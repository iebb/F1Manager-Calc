...
export function Calculator({ slot }) {
  ...
  // Record the current setup as an all-optimal previous run.
  const recordOptimal = () => {
    const pr = {
      track,
      carSetup: JSON.parse(JSON.stringify(carSetup)),
    ...
    update({ previousRuns: [pr, ...previousRuns] });
  }

  // Next Track: if this track has any feedback, mark the current setup as
  // optimal (and clear) before moving on; otherwise just advance.
  const nextTrack = () => {
    if (feedback.some(f => f.length > 0)) {
      recordOptimal();
      clearFeedbacks();
    }
    advanceTrack();
    ...
    <div key={slot.slotNaming} className="pt-3">
      ...
      <div className="grid grid-cols-1 items-start gap-3 lg:grid-cols-2">
        ...
        <Panel className="[&>*:last-child]:border-b-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex">
                <Select
                  ...
                  className="h-9 min-w-[200px] rounded-r-none"
                  ...
                  {["XX", ...seasonTrackOrders].map(tid => trackMap[tid]).map(t => (
                    <SelectItem key={t.id} value={t.id}>
                      <span className="flex items-center gap-2">
                        ...
                        <span>{t.name}{t.country && t.country !== "-" ? `, ${t.country}` : ""}</span>
                      </span>
                    </SelectItem>
                  ))}
                </Select>
                <Button
                  variant="success"
                  title="Next track (marks current as optimal if there's feedback)"
                  aria-label="Next track"
                  className="-ml-px h-9 rounded-l-none px-2.5"
                  onClick={nextTrack}
                >
              ...
              <div key={row.name} className="grid grid-cols-[110px_1fr_84px] items-center gap-2 border-b border-line/60 px-3 py-1.5 sm:grid-cols-[150px_1fr_96px]">
                <div>
                  ...
                  {currentTrack.perfectSetups[row.index][1] <= 1 && (
                    ...
                    >
                      <FlagIcon
                        code={currentTrack.id}
                        width={22} height={15} title={currentTrack.country}
          ...
          {(allowBiasEdit || allowBiasInput) && !isValidSetup.every(x => x) && (
          ...
          {/* Random / Preset actions (footer, to align with the feedback side) */}
          <div className="flex flex-wrap gap-2 border-t border-line px-3 py-2.5">
            <Button variant="danger" size="lg" className="min-w-[144px]" onClick={() => setCarSetup(randomSetup())}>Random</Button>
            <Button variant="secondary" size="lg" className="min-w-[144px]" onClick={loadPreset}>Load Preset</Button>
          </div>
        </Panel>
        ...
        <Panel>
          ...
          {/* Column headers */}
          <div className={`grid ${biasGridCols} gap-3 border-b border-line px-3 py-2 text-xs font-semibold uppercase tracking-wider text-zinc-400`}>
            <div />
            <div>Feedback</div>
            <div>Bias</div>
            {allowBiasInput && <div>Value</div>}
          </div>
          ...
          {BiasParams.map(row => {
            ...
            return (
              <div key={row.name + ":" + slot.slotNaming} className={`grid ${biasGridCols} items-center gap-3 border-b border-line/60 px-3 py-2.5`}>
                <div className="font-bold">{row.name}</div>
                {feedbackDropdown ? (
                  <Select
                    ...
                    onValueChange={(v) => createFeedback(row.index, biasValue, v)}
                  >
                    <SelectItem value="optimal">Optimal</SelectItem>
                    <SelectItem value="great">Great</SelectItem>
                    <SelectItem value="good">Good</SelectItem>
                    <SelectItem value="bad">Bad</SelectItem>
                    {allowBadDir && <SelectItem value="bad+">Bad ↑</SelectItem>}
                    {allowBadDir && <SelectItem value="bad-">Bad ↓</SelectItem>}
                  </Select>
                ) : (
                  <FeedbackCycle
                ...
                {allowBiasInput && (
                  <Input
                    type="number"
                    value={biasParam[row.index].toFixed(3)}
                    inputMode="numeric"
                    step={0.001}
                    min={0}
                    max={1}
                    className="h-9 px-2 no-spinner"
                    onChange={(e) => {
                      const nVal = Number(e.target.value);
                      if (e.target.value !== "" && 0 <= nVal && nVal <= 1) {
          ...
          <div className="flex flex-wrap items-center gap-3 border-t border-line p-3">
            ...
            <Button variant="primary" size="lg" className="ml-auto min-w-[144px]" onClick={findNearest}>
              Find Setup →
            </Button>
          </div>
        </Panel>
                  ...
                  {previousRuns.map(x => (
                    <tr key={x.id} className="border-b border-line/50">
                      <td className="whitespace-nowrap px-2 py-1.5">
                        <span className="flex items-center gap-2">
                          <FlagIcon code={x.track} width={24} height={20} title={x.track} />
